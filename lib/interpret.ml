...
(*(1** Extracts list of rules from ast. Keeps rules order *1) *)
(*let rules_of_ast : ast -> rule list = *)
(*  let rule_of_expr = function *)
(*    | Rule rule -> Some rule *)
(*    (1* For now expr could only be a Rule, so we don't need line below *1) *)
(*    (1* | _ -> None *1) *)
(*  in *)
(*  function *)
(*  | rule, [] -> [ rule ] *)
...
(*(1** Original rule type has tuple as "targets" type, *)
(*But it is easier to works with lists *1) *)
(*type rule_as_lists = *)
(*  { targets : string list *)
...
(*(1** Transform list of ast's rules to rule_as_lists list *)
(* Keeps rules order *1) *)
...
(*(1* Return true if a target is present somewhere in targets in one rule *1) *)
(*let rule_has_target target rule = List.mem target rule.targets *)

(*(1* Return true if a target is present somewhere in targets in list of rules *1) *)
(*let rules_has_target target rules = *)
(*  Option.is_some (List.find_opt (rule_has_target target) rules) *)
(*;; *)

(*(1** Return recipes for specified target among all rules. *)
(*If two or more rules are found with the same target in the target list, *)
(*and with different recipes, the newer one will be used *1) *)
...
(*(1** Return dependencies for specified target among all rules. *1) *)
...
(*(1* Get unique filenames occuring in targets and dependencies among all rules *1) *)
...
(*(1* This is mutable data structure. *)
...
(*(1* TODO: Use label (Unit) somehow. Probably mark PHONY targets with it *1) *)
...
(*(1* Map to get corresponding vertex in graph by label (string), *)
(*   cause Graphlib do not provide such interface. *1) *)
(*type vertex_of_target = G.Node.t VertexMap.t *)

(*(1* Creates new vertex and puts in into the map. *)
(*   If it's already present, simply returns that vertex *1) *)
...
(*(1* Constructs graph and map to get corresponding vertex by target's name *1) *)
(*(1* Check is the filename is empty on substitution, return error *1) *)
...
(*(1* Traverse @targets, addding corresponding nodes to graph if they do not exist *1) *)
...
(*(1* Set of nodes that require recompiling *1) *)
(*type dfs_state = *)
(*  { nodes_to_recompile : G.Node.Set.t *)
(*  ; finish_component : bool *)
(*  } *)

(*(1* fold on @n predecessors in graph @g. Loops are ignored *1) *)
...
(*(1* Returns true if for at least one parent of X *)
(*   timestamp (X) > timestamp (Parent), or Parent does not exist  *1) *)
...
(*(1* Traversing **only one** component starting from @node in a *)
(*   depth-first-search manner, applying f when leaving node. *)
(*   Loops are ignored and info about them are printed to stdout. *1) *)
...
(*(1* Could raise exception *1) *)
(*let build_target target map graph rules = *)
(*  (1* This will find a node cause all possible targets should be *)
...
(*  in *)
(*  match targets with *)
(*  (1* When no targets are specified, choose first one *1) *)
(*  | [] -> traverse_targets [ rules |> get_unique_filenames |> List.hd ] *)
(*  | _ -> traverse_targets targets *)
(*;; *)

(*(1* Usage: *1) *)
(*(1* let file = open_out_bin "mygraph.dot" in *1) *)
(*(1* Dot.output_graph file graph; *1) *)
...
(*    (1* [ `Shape `Box; (if v.node_label then `Color 16711680 else `Color 0) ] *1) *)
(*    [ `Shape `Box ] *)
(*  ;; *)
